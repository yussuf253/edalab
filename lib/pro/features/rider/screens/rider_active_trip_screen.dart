...
class _RiderActiveTripScreenState extends State<RiderActiveTripScreen> {
  ...
  Future<void> _advanceStatus() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l10n.rideStatusUpdated(_statusLabel(nextStatus))),
        ),
      );
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.couldNotUpdateRide(error.toString()))),
      );
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final destination =
        ride?['destination']?.toString() ?? l10n.dropoffLocationFallback;
    final driverName =
        ride?['driverName']?.toString() ?? l10n.assignedRiderFallback;
    final vehicle = ride?['vehicle']?.toString() ?? l10n.vehicleLabel;
    final status = ride?['status']?.toString().toUpperCase() ?? 'ACCEPTED';
    final passengerName =
        ride?['customerName']?.toString() ?? l10n.passengerLabel;
