...
function defaultLaundryCatalogBookingConfig(basePrice) {
    const washAndFoldBasePrice = Number.isFinite(basePrice) && basePrice > 0 ? basePrice : 6000;
    return {
        itemCatalog: [
...
function normalizeLaundryCatalogBookingConfig(value, fallback) {
    ...
    const pickupSlots = pickupSlotsRaw == null
        ...
        : readJsonStringArray(pickupSlotsRaw).map((entry) => entry.trim()).filter((entry) => entry.length > 0).slice(0, 12);
...
router.get('/health-services/categories', (0, async_handler_1.asyncHandler)(async (req, res) => {
    const categories = await db_1.prisma.healthServiceCategory.findMany({
        where: { active: true },
        orderBy: { sortOrder: 'asc' },
        select: {
            id: true,
...
router.get('/health-services/tests', (0, async_handler_1.asyncHandler)(async (req, res) => {
    const { categoryId } = req.query;
    const where = {
        active: true,
        ...(categoryId ? { categoryId: categoryId } : {}),
    };
    const tests = await db_1.prisma.labTest.findMany({
        where,
        orderBy: { sortOrder: 'asc' },
        include: {
            category: {
