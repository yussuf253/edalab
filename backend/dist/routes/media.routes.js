...
const uploadScopeSchema = zod_1.z.enum([
    'user_avatar',
    'pro_avatar',
    'store',
    'product',
...
const uploadMediaSchema = zod_1.z.object({
    ...
    ownerId: zod_1.z.string().trim().min(1).max(120).optional().nullable(),
    fileName: zod_1.z.string().trim().min(1).max(180).optional().nullable(),
    mimeType: zod_1.z.string().trim().max(120).optional().nullable(),
...
function decodedBase64Payload(dataBase64) {
...
function bucketForScope(scope) {
    if (scope === 'user_avatar' || scope === 'pro_avatar') {
        return env_1.env.SUPABASE_STORAGE_BUCKET_AVATARS.trim() || 'avatars';
    }
    if (scope === 'prescription') {
        return env_1.env.SUPABASE_STORAGE_BUCKET_PRESCRIPTIONS.trim() || 'prescriptions';
    }
    return env_1.env.SUPABASE_STORAGE_BUCKET_MEDIA.trim() || 'media';
}
function objectPrefixForScope(scope, ownerSegment) {
    switch (scope) {
        case 'user_avatar':
...
async function uploadToSupabaseStorage(params) {
    ...
    const encodedObjectPath = params.objectPath
        ...
        .join('/');
    const uploadUrl = `${supabaseUrl.replace(/\/+$/g, '')}/storage/v1/object/${encodeURIComponent(params.bucket)}/${encodedObjectPath}`;
    ...
    if (!response.ok) {
        ...
        throw new Error(`Supabase media upload failed (${response.status}): ${rawBody.slice(0, 300)}`);
    }
    return true;
}
router.post('/upload', (0, async_handler_1.asyncHandler)(async (req, res) => {
    ...
    const extension = (extFromMime || extFromName || 'jpg').replace(/[^a-z0-9]/gi, '') || 'jpg';
    ...
    try {
        ...
        if (uploaded) {
            const encodedObjectPath = objectPath
                ...
                .map((segment) => encodeURIComponent(segment))
                .join('/');
            publicUrl = buildPublicUrl(req, `/uploads/supabase/${encodeURIComponent(bucket)}/${encodedObjectPath}`);
            storedPath = `supabase://${bucket}/${objectPath}`;
        }
        else {
            const absolutePath = path_1.default.resolve(process.cwd(), 'uploads', bucket, ...objectPath.split('/'));
